create or replace trigger trg_IUTareasTTareaComprobantes
...
begin
	case 
		...
		when updating then
			...
			if :new.trs_id_padre is not null then
				if :new.trs_id_padre <> :old.trs_id_padre then
					SELECT TRS_ID||'-'||(SELECT set_descripcion FROM   services_types s
															WHERE  s.set_id = t2.set_id)||'-'|| DTA_CELL_ID_A||'-'||DTA_CELL_ID_B into tareaPadreVar
						FROM   detalles_tareas t2  WHERE  t2.trs_id = :new.trs_id_padre;
				...
				update TareasComprobantes set tarea_padre = null where numero = :old.trs_id;
			end if;
	end case;
end trg_IUTareasTTareaComprobantes;	