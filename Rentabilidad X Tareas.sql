...
insert into rentabilidadxtareas select t.trs_id,(select sum(dc.dco_importe) from tareas_detalles_comprobantes tdc join detalles_comprobantes dc on 
	...
	and (t.trs_id in (select tdc.trs_id from tareas_detalles_comprobantes tdc join detalles_comprobantes dc on 
	tdc.cmp_numero = dc.cmp_numero and	tdc.dco_numero = dc.dco_numero join comprobantes c on c.cmp_numero = dc.cmp_numero 
...
update rentabilidadxtareas r set ingreso = ingreso + (select SUM(TRUNC(cmp_importe_neto/(select count(*) from tareas t where extract(month from t.trs_fecha_fin) = 
	...
	EXTRACT( YEAR FROM ce.CMP_FECHA_EMISION)) where r.trs_id in (select distinct trs_id from tareas te, comprobantes c join detalles_comprobantes dc 
	on c.cmp_numero = dc.cmp_numero where  
